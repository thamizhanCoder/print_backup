...
class EmployeeController extends Controller
{
    ...
    {
        $get_dept = Department::where('status', 1)->select('department.*')->get();
    ...
    public function employee_create(EmployeeRequest $request)
    {
        try {
            ...
            if (!empty($request->employee_image)) {
                $Extension =  pathinfo($request->input('employee_image'), PATHINFO_EXTENSION);
            ...
            if (empty($mobileexist)) {
                ...
                if (empty($emailexist)) {
                    ...
                    if ($employee->save()) {
                        ...
                        $name =  env('EMPLOYEE_LOGIN_URL');

                        //mail send
                        $mail_data = [];
                        $mail_data['link'] = $name; 
                        $mail_data['employee_name'] = $employees->employee_name;
                        $mail_data['password'] = $request->password;
                        $mail_data['email'] = $employees->email;

                        if ($employees->email != '') {
                            event(new EmployeeCreate($mail_data));
                        }
        ...
        try {
            ...
            if ($count > 0) {
                ...
                foreach ($employees as $value) {
                    ...
                    $taskEmployee = EmployeeTaskHistory::where('employee_id', $value['employee_id'])->where('employee_status', '!=', 2)->first();
                    $ary['work_status'] = !empty($taskEmployee) ? $taskEmployee->status : null;
            ...
            if (!empty($final)) {
                ...
                return response()->json([
                    'keyword' => 'success',
                    'message' => __('Employees listed successfully'),
                    'data' => $final,
                    'count' => $count
                ]);
            } else {
        ...
        try {
            ...
            if (empty($mobileexist)) {
                ...
                if (empty($emailexist)) {
                    ...
                    $employeesOldDetails = Employee::where('employee_id', $request->input('employee_id'))->first();
                    ...
                    if ($employee->save()) {
                        $employees = Employee::where('employee_id', $employee->employee_id)->first();
                        // log activity
                        $desc =  'Employee ' . '(' . $employeesOldDetails->employee_name . ')' . ' is updated as ' . '(' . $employee->employee_name . ')' . ' by ' .JwtHelper::getSesUserNameWithType() . '';
                        $activitytype = Config('activitytype.Employee');
                        GlobalHelper::logActivity($desc, $activitytype, JwtHelper::getSesUserId(), 1);
    ...
    public function employee_view($id)
    {
        try {
            ...
            if (!empty($employee_view)) {
                ...
                $ary['employee_image_url'] = ($employee_view['employee_image'] != '') ? env('APP_URL') . env('EMPLOYEE_URL') . $employee_view['employee_image'] : env('APP_URL') . "avatar.jpg";
                $ary['employee_image'] = $employee_view['employee_image'];
