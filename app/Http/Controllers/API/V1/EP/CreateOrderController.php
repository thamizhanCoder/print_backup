...
class CreateOrderController extends Controller
{
    ...
    {
        try {
            Log::channel("employeeCreateOrder")->info('** started the bulkOrderCreate method **');
            ...
            $order->created_by = JwtHelper::getSesEmpUserId();
            ...
            if ($order->save()) {
                ...
                if (!empty($quoteJsonDetails)) {
                    ...
                    if ($quoteUpdateDetails == true) {
                        ...
                        Log::channel("employeeCreateOrder")->info($quoteUpdateDetailsMessage);
                    } else {
                ...
                $updateStatusQuote->updated_by = JwtHelper::getSesEmpUserId();
                $updateStatusQuote->save();
                Log::channel("employeeCreateOrder")->info("update the enquiry status save value :: $updateStatusQuote");
                ...
                $updateStatusEnquiry->updated_by = JwtHelper::getSesEmpUserId();
                $updateStatusEnquiry->save();
                Log::channel("employeeCreateOrder")->info("update the enquiry status save value :: $updateStatusEnquiry");
                ...
                $updateStatusEnquiryHistory->acl_user_id = JwtHelper::getSesEmpUserId();
                $updateStatusEnquiryHistory->save();
                Log::channel("employeeCreateOrder")->info("track history table save value :: $updateStatusEnquiryHistory");
                ...
                Log::channel("employeeCreateOrder")->info("quote bulkOrderCreate order save value :: $order");
                Log::channel("employeeCreateOrder")->info('** end the bulkOrderCreate method **');
            ...
            Log::channel("employeeCreateOrder")->info('** start the bulkOrderCreate error method **');
            Log::channel("employeeCreateOrder")->error($exception);
            Log::channel("employeeCreateOrder")->info('** end the bulkOrderCreate error method **');
    ...
    {
        try {
            Log::channel("employeeCreateOrder")->info('** started the bulkOrderUpdate method **');
            ...
            $order->updated_by = JwtHelper::getSesEmpUserId();
            ...
            if ($order->save()) {
                ...
                if (!empty($orderItemJsonDetails)) {
                    ...
                    if ($orderItemUpdateDetails == true) {
                        ...
                        Log::channel("employeeCreateOrder")->info($orderItemUpdateDetailsMessage);
                    } else {
                ...
                GlobalHelper::logActivity($desc, $activitytype, JwtHelper::getSesEmpUserId(), 1);

                Log::channel("employeeCreateOrder")->info("quote bulkOrderUpdate order save value :: $order");
                Log::channel("employeeCreateOrder")->info('** end the bulkOrderUpdate method **');
            ...
            Log::channel("employeeCreateOrder")->info('** start the bulkOrderUpdate error method **');
            Log::channel("employeeCreateOrder")->error($exception);
            Log::channel("employeeCreateOrder")->info('** end the bulkOrderUpdate error method **');
    ...
    public function updateQuoteAttachment(updateQuoteAttachmentRequest $request)
    {
        try {
            ...
            Log::channel("employeeCreateOrder")->info('** started the updateQuoteAttachment method **');
            ...
            if ($quoteDetail->service_id == 1) {
                ...
                Log::channel("employeeCreateOrder")->info("quote updateQuoteAttachment passport request value :: $request->images, $request->background_color");
            } else if ($quoteDetail->service_id == 2) {
                $updateQuoteDetails->photoprint_variant = $images;
                Log::channel("employeeCreateOrder")->info("quote updateQuoteAttachment photoprint request value :: $images");
            } else if ($quoteDetail->service_id == 3) {
                $updateQuoteDetails->frames = $images;
                Log::channel("employeeCreateOrder")->info("quote updateQuoteAttachment phtoframe request value :: $images");
            } else if ($quoteDetail->service_id == 4) {
                $updateQuoteDetails->variant_attributes = $images;
                Log::channel("employeeCreateOrder")->info("quote updateQuoteAttachment personalized request value :: $images");
            } else if ($quoteDetail->service_id == 6) {
                $updateQuoteDetails->images = $images;
                Log::channel("employeeCreateOrder")->info("quote updateQuoteAttachment selfie request value :: $images");
            }
            ...
            if ($updateQuoteDetails->save()) {
                ...
                Log::channel("employeeCreateOrder")->info("quote updateQuoteAttachment save value :: $updateQuoteDetails");
                Log::channel("employeeCreateOrder")->info('** end the updateQuoteAttachment method **');
            ...
            Log::channel("employeeCreateOrder")->info('** start the updateQuoteAttachment error create method **');
            Log::channel("employeeCreateOrder")->error($exception);
            Log::channel("employeeCreateOrder")->info('** end the updateQuoteAttachment error create method **');
    ...
    {
        try {
            Log::channel("employeeCreateOrder")->error('**started the createOrderview method**');
            ...
            Log::channel("employeeCreateOrder")->error('**started the error createOrderview method**');
            Log::channel("employeeCreateOrder")->error($exception);
            Log::channel("employeeCreateOrder")->error('**ended the error createOrderview method**');
            return response()->json([
                'error' => 'Internal server error.',
                'message' => $exception->getMessage()
            ], 500);
