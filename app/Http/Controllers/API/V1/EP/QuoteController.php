...
class QuoteController extends Controller
{
    ...
    {
        try {
            Log::channel("employeeQuote")->info('** start the serviceGetAll method **');
            ...
            Log::channel("employeeQuote")->info('** start the serviceGetAll error method **');
            Log::channel("employeeQuote")->error($exception);
            Log::channel("employeeQuote")->info('** end the serviceGetAll error method **');
    ...
    {
        try {
            Log::channel("employeeQuote")->info('** started the quote create method **');
            ...
            $quote->created_by = JwtHelper::getSesEmployeeId();
            ...
            if ($quote->save()) {
                ...
                if (!empty($quoteDetails)) {
                    ...
                    if ($quoteSaveDetails == true) {
                        ...
                        Log::channel("employeeQuote")->info($quoteSaveDetailsMessage);
                    } else {
                ...
                $updateStatusEnquiry->updated_by = JwtHelper::getSesEmployeeId();
                $updateStatusEnquiry->save();
                Log::channel("employeeQuote")->info("update the enquiry status save value :: $updateStatusEnquiry");
                ...
                $updateStatusEnquiryHistory->portal_type = 2;
                $updateStatusEnquiryHistory->created_on = Server::getDateTime();
                $updateStatusEnquiryHistory->employee_id = JwtHelper::getSesEmployeeId();
                $updateStatusEnquiryHistory->save();
                Log::channel("employeeQuote")->info("insert the enquiry history save value :: $updateStatusEnquiryHistory");

                Log::channel("employeeQuote")->info("quote main table save value :: $quote");
                Log::channel("employeeQuote")->info('** end the quote update method **');
            ...
            Log::channel("employeeQuote")->info('** start the quote error create method **');
            Log::channel("employeeQuote")->error($exception);
            Log::channel("employeeQuote")->info('** end the quote error create method **');
    ...
    {
        try {
            Log::channel("employeeQuote")->info('** start the serviceBasedProductList method **');
            ...
            Log::channel("employeeQuote")->info('** start the serviceBasedProductList error method **');
            Log::channel("employeeQuote")->error($exception);
            Log::channel("employeeQuote")->info('** end the serviceBasedProductList error method **');
    ...
    {
        try {
            Log::channel("employeeQuote")->info('** start the productfilter method **');
            ...
            Log::channel("employeeQuote")->info('** start the productfilter error method **');
            Log::channel("employeeQuote")->error($exception);
            Log::channel("employeeQuote")->info('** end the productfilter error method **');
    ...
    {
        try {
            Log::channel("employeeQuote")->info('** start the termsAndCondition method **');
            ...
            Log::channel("employeeQuote")->info('** start the termsAndCondition error method **');
            Log::channel("employeeQuote")->error($exception);
            Log::channel("employeeQuote")->info('** end the termsAndCondition error method **');
    ...
    {
        try {
            Log::channel("employeeQuote")->info('** started the quote list method **');
            ...
            $quoteDetails = BulkOrderQuote::select('bulk_order_quote.*', 'bulk_order_enquiry.contact_person_name', 'bulk_order_enquiry.mobile_no', 'bulk_order_enquiry.email', 'bulk_order_enquiry.customer_type', 'bulk_order_enquiry.company_name')->leftjoin('bulk_order_enquiry', 'bulk_order_enquiry.bulk_order_enquiry_id', '=', 'bulk_order_quote.bulk_order_enquiry_id')->where('bulk_order_enquiry.employee_id',  $getEmployeeId);
            ...
            if (!empty($final)) {
                ...
                Log::channel("employeeQuote")->info("list value :: $log");
                Log::channel("employeeQuote")->info('** end the quote list method **');
                return response()->json([
                    'keyword' => 'success',
            ...
            Log::channel("employeeQuote")->error($exception);
            Log::channel("employeeQuote")->error('** end the quote list method **');
    ...
    {
        Log::channel("employeeQuote")->info('** started the admin enquiry list excel method **');
    ...
    {
        try {
            Log::channel("employeeQuote")->error('**started the quoteView method**');
            ...
            Log::channel("employeeQuote")->error('**started the error quoteView method**');
            Log::channel("employeeQuote")->error($exception);
            Log::channel("employeeQuote")->error('**ended the error quoteView method**');
            return response()->json([
                'error' => 'Internal server error.',
                'message' => $exception->getMessage()
            ], 500);
    ...
    {
        try {
            Log::channel("employeeQuote")->info('** started the quoteUpdateStatus update method **');
            $updateStatus = BulkOrderQuote::find($request->bulk_order_quote_id);
            $updateStatus->status = $request->status;
            if($request->status == 2){
            $updateStatus->is_reraised = 1;
            }
            $updateStatus->notes = $request->notes;
            $updateStatus->updated_on = Server::getDateTime();
            $updateStatus->updated_by = JwtHelper::getSesEmployeeId();
            if ($updateStatus->save()) {
                Log::channel("employeeQuote")->info("Updated the BulkOrderQuote table in status :: $updateStatus");

                //Bulk order enquiry table update
                $enquiryStatusUpdate = BulkOrderEnquiry::find($updateStatus->bulk_order_enquiry_id);
                if ($request->status == 2) {
                    Log::channel("employeeQuote")->info('** Updated the enquiry table status Quote Request for reraise **');
                    $status = 7;
                } else if ($request->status == 4) {
                    Log::channel("employeeQuote")->info('** Updated the enquiry table status Quote Approved **');
                    $status = 9;
                } 
                else if ($request->status == 5) {
                    Log::channel("employeeQuote")->info('** Updated the enquiry table status Quote Rejected **');
                    $status = 10;
                }
                $enquiryStatusUpdate->status = $status;
                $enquiryStatusUpdate->updated_on = Server::getDateTime();
                $enquiryStatusUpdate->updated_by = JwtHelper::getSesEmployeeId();
                $enquiryStatusUpdate->save();
                Log::channel("employeeQuote")->info("Updated the enquiry table in status :: $enquiryStatusUpdate");
                ...
                $updateStatusEnquiry->save();
                Log::channel("employeeQuote")->info("Updated the enquiry track history table in status :: $updateStatusEnquiry");

                $getStatus = BulkOrderEnquiryStatus::where('bulk_order_enquiry_status_id', $enquiryStatusUpdate->status)->first();

                Log::channel("employeeQuote")->info('** end the quoteUpdateStatus update method **');
                if ($request->status == 2) {
                    ...
                    $quoteReasonUpdate->employee_id = JwtHelper::getSesEmployeeId();
                    $quoteReasonUpdate->created_on = Server::getDateTime();
                    $quoteReasonUpdate->save();
                    
                    //Send Mail
                    $quoteSaveDetails = $this->quoterequestEmail($request->bulk_order_quote_id);

                    //Push Notification 
                    $get_admin_token = UserModel::where('acl_user_id',1)->where('token', '!=', NULL)->select('token')->first();
                    $get_quote_code = BulkOrderQuote::where('bulk_order_quote_id',$request->bulk_order_quote_id)->first();
                    $get_employee_id = BulkOrderEnquiry::where('bulk_order_enquiry_id',$get_quote_code->bulk_order_enquiry_id)->first();
                    $get_employee_details = Employee::where('employee_id',$get_employee_id->employee_id)->first();

                        $title = "Request for quote reraise"." - ".$get_quote_code->quote_code;
                        $body = "You have received a request for quote re-raised $get_quote_code->quote_code raised by $get_employee_details->employee_name";
                        $module = 'Quote reraise';
                        $page = 'quote_reraised_request';
                        $portal = 'admin';
                        ...
                        $data = [
                            ...
                            'platform' => "admin",
                            'quote_code'=>$get_quote_code->quote_code,
                    ...
                    return response()->json(['keyword' => 'success', 'message' => ('Request for reraise successfully'), 'data' => []]);
                } else if ($request->status == 4) {
            ...
            Log::channel("employeeQuote")->error('**starts error occured in quoteUpdateStatus update method **');
            Log::channel("employeeQuote")->error($exception);
            Log::channel("employeeQuote")->error('**end error occured in quoteUpdateStatus update method **');
            return response()->json([
                'error' => 'Internal server error.',
                'message' => $exception->getMessage()
            ], 500);
    ...
    public function quoterequestEmail($getQuoteId)
    {
            //mail send
        $get_quote_code = BulkOrderQuote::where('bulk_order_quote_id',$getQuoteId)->first();
        $get_employee_id = BulkOrderEnquiry::where('bulk_order_enquiry_id',$get_quote_code->bulk_order_enquiry_id)->first();
        $get_employee_details = Employee::where('employee_id',$get_employee_id->employee_id)->first();
        ...
        if($get_admin_email->email != null)
        {
            ...
            $mail_data['quote_code'] = $get_quote_code->quote_code;
            $mail_data['quote_description'] = $get_quote_code->notes;
            $mail_data['email'] = $get_admin_email->email;

            if ($get_admin_email->email != '') {

                event(new QuoteRequestEmployee($mail_data));
        }
        }
