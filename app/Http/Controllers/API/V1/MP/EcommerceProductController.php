...
class EcommerceProductController extends Controller
{
    ...
    public function ecommerceList(Request $request)
    {
        ...
        $data->save();
            Log::channel("ecommerce")->info('** started the ecommerce mobile list method **');
            ...
            if (!empty($final)) {
                ...
                Log::channel("ecommerce")->info('** end the ecommerce mobile list method **');
            ...
            Log::channel("ecommerce")->error('** end the ecommerce mobile list method **');
    ...
    {
        try {
            Log::channel("ecommerce")->info('** started the ecommerce mobile view method **');
            if ($id != '' && $id > 0) {
                ...
                if (!empty($final)) {
                    // $log = json_encode($final, true);
                    // Log::channel("ecommerce")->info("view mobile value :: $log");
                    return response()->json([
                        'keyword' => 'success',
            ...
            Log::channel("ecommerce")->error($exception);
            Log::channel("ecommerce")->info('** end the ecommerce mobile view method **');
            return response()->json([
                'error' => 'Internal server error.',
                'message' => $exception->getMessage(),
            ], 500);
        ...
        try {
            ...
            if (empty($check)) {
                ...
                if ($cart->save()) {
                    ...
                    return response()->json([
                        'keyword'      => 'failed',
                        'message'      => __('Product added to cart failed'),
                        'data'        => []
                    ]);
                }
            } else {
    ...
    public function ratingReviewProductList(Request $request, $id)
    {
        ...
        $ratingReviewProduct->orderBy('rating_review.created_on', 'desc');
