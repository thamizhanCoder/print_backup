...
use App\Http\Requests\OtherDistrictCreateRequest;
...
class LocationController extends Controller
{
    ...
    public function state(Request $request)
    {
        ...
        if (!empty($get_state)) {
            return response()->json([
                'keyword' => 'success',
                'message' => 'State viewed successfully',
                'data' => $get_state,
            ]);
        } else {
            return response()->json([
    ...
    public function country(Request $request)
    {
        ...
        if (!empty($get_country)) {
            return response()->json([
                'keyword' => 'success',
                'message' => 'Country viewed successfully',
                'data' => $get_country,
            ]);
        } else {
            return response()->json([
    ...
    public function district(Request $request)
    {
        ...
        if (!empty($get_district)) {
            return response()->json([
                'keyword' => 'success',
                'message' => 'District viewed successfully',
                'data' => $get_district,
                'count' => $count
            ]);
        } else {
    ...
    public function otherDistrict(OtherDistrictCreateRequest $request)
    {
        $otherDistrict = new OtherDistrict();
        $otherDistrict->district = $request->district_name;
        ...
        if ($otherDistrict->save()) {
            $otherDistrict_details = OtherDistrict::where('other_district_id', $otherDistrict->other_district_id)->first();

            return response()->json([
                'keyword'      => 'success',
                'message'      => __('Other district created successfully'),
                'data'        => [$otherDistrict_details]
            ]);
        } else {
            return response()->json([
                'keyword'      => 'failure',
                'message'      => __('Other district creation failed'),
                'data'        => []
            ]);
        }
    }
