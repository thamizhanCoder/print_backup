...
class MyProfileController extends Controller
{
    ...
    {
        try {
            Log::channel("mobilemyprofile")->info('** started the mobilemyprofile update method **');
            ...
            if (empty($email)) {
                ...
                if (empty($mobile)) {
                    ...
                    Log::channel("mobilemyprofile")->info("request value :: $customer->customer");
                    ...
                    if ($customer->save()) {
                        ...
                        $final = [];
                        $ary = [];
                        $ary['customer_id'] = $customers['customer_id'];
                        $ary['customer_first_name'] = $customers['customer_first_name'];
                        $ary['customer_last_name'] = $customers['customer_last_name'];
                        $ary['mobile_no'] = $customers['mobile_no'];
                        $ary['email'] = $customers['email'];
                        $ary['image'] = $customers['profile_image'];
                        $ary['profile_image'] = ($customer['profile_image'] != '') ? env('APP_URL') . env('PROFILE_URL') . $customer['profile_image'] : env('APP_URL') . "avatar.jpg";
                        $ary['check_profile_image'] = $customers['check_profile_image'];
                        $ary['updated_on'] = $customers['updated_on'];
                        $ary['updated_by'] = $customers['updated_by'];
                        $final = $ary;

                        Log::channel("mobilemyprofile")->info("save value :: $customers");
                        Log::channel("mobilemyprofile")->info('** end the mobilemyprofile update method **');
                        if (!empty($final)) {
                            return response()->json([
                                'keyword'      => 'success',
            ...
            Log::channel("mobilemyprofile")->error($exception);
            Log::channel("mobilemyprofile")->error('** end the profile update method **');
        ...
        try {
            ...
            if ($customer->save()) {
                ...
                return response()->json([
                    'keyword'      => 'failed',
                    'data'        => [],
                    'message'      => __('Delivery address updated failed')
                ]);
            }
        }
            else{
