...
class OrderPlaceController extends Controller
{
  ...
  {
    // try {
    Log::channel("ordermobile")->info('** started the order create method **');
    ...
    if ($order_items != '[]') {
      ...
      if ($order->save()) {
        ...
        Log::channel("ordermobile")->info("customer details :: $order");
        Log::channel("ordermobile")->info('** end the order customer details **');
        ...
        if (!empty($order_items)) {
          for ($i = 0; $i < count($order_items); $i++) {
            ...
            if ($order_data->save()) {
              ...
              Log::channel("ordermobile")->info("order items details :: $order_data");
              Log::channel("ordermobile")->info('** end the order items details **');
              //change
              ...
              if (!empty($cartQuantity)) {
                Log::channel("ordermobile")->info("cartQuantity :: $cartQuantity");
              }
              Log::channel("ordermobile")->info("product quantity :: $product->quantity");
              // Log::channel("ordermobile")->info("remaining product quantity :: $quantity");
              Log::channel("ordermobile")->info('** end the order items quantity **');
              $addtocartId = $order_items[$i]['add_to_cart_id'];
              if (!empty($addtocartId)) {
                Log::channel("ordermobile")->info("addtocartId :: $addtocartId");
                Log::channel("ordermobile")->info('** end the addtocartId **');
              }
          ...
          if ($request->input('is_cod') == 2) {
            if ($order->paytm_payment_status == "success") {
        ...
        Log::channel("ordermobile")->info("final order details :: $order_det");
        Log::channel("ordermobile")->info('** end the order details method **');
      ...
      $orders_notify_data = Orders::where('orders.order_id', $order->order_id)->leftJoin('customer','customer.customer_id','=','orders.created_by')->select('customer.customer_first_name', 'customer.customer_last_name', 'orders.created_by', 'orders.order_code', 'orders.order_id', 'orders.customer_id')->orderBy('orders.created_on','desc')->first();    
      $customerName = !empty($orders_notify_data->customer_last_name) ? $orders_notify_data->customer_first_name . ' ' . $orders_notify_data->customer_last_name : $orders_notify_data->customer_first_name;
      $module = "Place Order";
      ...
      $admin_reciever = UserModel::where('token', '!=', " ")->where('acl_role_id', 1)->first();
    ...
    //   Log::channel("ordermobile")->error($exception);
    //   Log::channel("ordermobile")->error('** end the order create method **');
  ...
  public function outofstockPushNotification($quantity, $product)
  {
    ...
    if ($qnty_check == 0) {
      ...
      $body = "$product->product_name is out of stock now. Check & fill the 
        stock.";
      $random_id = substr(str_shuffle("0123456789abcdefghijklmnopqrstvwxyz"), 0, 9);
      $module = 'Out of stock';
      $portal = "admin";
      ...
      $token = UserModel::where('acl_role_id', 1)->where('token', '!=', NULL)->first();
      if (!empty($token)) {
        $message = [
          'title' => $title,
          'body' => $body,
