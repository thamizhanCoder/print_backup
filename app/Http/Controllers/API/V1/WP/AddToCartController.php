...
class AddToCartController extends Controller
{
    ...
    {
        $notify = Notify::where([
            ['email', '=', $request->input('email')], ['product_id', '=', $request->product_id], ['product_variant_id', '=', $request->product_variant_id],
            ['status', '!=', 2]
        ])->first();
        if (empty($notify)) {
            $notify = new Notify();
            ...
            $notify->customer_registered_from  = "web";
    ...
    {
        try {
            Log::channel("addtocart_web")->info('** started the addtocart delete method **');
            ...
            if (!empty($id)) {
                ...
                Log::channel("addtocart_web")->info("delete add_to_cart_id value :: $request->add_to_cart_id");
                Log::channel("addtocart_web")->info("delete customer_id value :: $addToCartdetails->customer_id");
                Log::channel("addtocart_web")->info('** end the addtocart delete method **');
            ...
            Log::channel("addtocart_web")->error($exception);
            Log::channel("addtocart_web")->error('** end the addtocart delete method **');
    ...
    //         'p.category_id',
    //         'c.category_name',
    //         'pv.variant_code as product_code',
    //         'p.gst_percentage as gst_percentage_id',
    //         'gst_percentage.gst_percentage',
    //         'service.delivery_charge',
    //         'p.is_cod_available',
    ...
    //                 [
    //                     'keyword' => 'failed',
    //                     'message' => __('Checkout Quantity failed successfully'),
