...
class MyOrderController extends Controller
{
    ...
    public function myorder_list(Request $request)
    {
        try {
            ...
            foreach ($orders as $value) {
                ...
                // $cancelDetails = OrderItems::where('order_items.order_id', $value->order_id)->where('production_status', 1)->where('order_status', '!=', 4)->first();
    ...
    public function cancelOrderItem(Request $request)
    {
        try {
            ...
            if (!empty($id)) {
                ...
                $msg =  "Dear $customerName,Thanks for shopping with Print App!! We’re sorry that you’re unsatisfied with your order from Print App. Your order $itmesNames has been cancelled successfully. We have started the refund process for your order, the details will be updated soon. For more detail: $WEBSITE_URL.Team Print App";
    ...
    public function cancelOrder(Request $request)
    {
        ...
        $msg =  "Dear $customerName,Thanks for shopping with Print App!! We’re sorry that you’re unsatisfied with your order from Print App. Your order $order->order_code has been cancelled successfully. We have started the refund process for your order, the details will be updated soon. For more detail: $WEBSITE_URL.Team Print App";
        $isSmsSent = GlobalHelper::sendSMS($order->mobile_no, $msg);
        ...
        //Send Push Notification for Admin
        $pushNotificationforAdmin = $this->OrderCancelPushNotificationForAdmin($order->order_id);

        //Send Push Notification for Customer
        $pushNotificationforcustomer = $this->OrderCancelPushNotificationForCustomer($order->order_id);
    ...
    public function OrderCancelPushNotificationForAdmin($orderId)
    {
        //Send notification for admin 
        $user_id = JwtHelper::getSesUserId();

        $customer_details = Orders::where('order_id', $orderId)->first();

        $getCustomer = Customer::where('customer_id', $customer_details->customer_id)->first();

        $title = "Order Cancelled - $customer_details->order_code";
        $portal = 'admin';
        $module = "order_cancel";
        $random_id1 = substr(str_shuffle("0123456789abcdefghijklmnopqrstvwxyz"), 0, 9);
        $body = "The Order $customer_details->order_code is cancelled by $getCustomer->customer_first_name $getCustomer->customer_last_name";
        $page = "order_cancelled";
        $data = [
            'customer_id' => $customer_details->customer_id,
            'customer_name' => !empty($getCustomer->customer_last_name) ? $getCustomer->customer_first_name . ' ' . $getCustomer->customer_last_name : $getCustomer->customer_first_name,
        ...
        $getdata = GlobalHelper::notification_create($title, $body, 1, $user_id, 1, $module, $page, $portal, $data, $random_id1);
    }

    public function OrderCancelPushNotificationForCustomer($ord_id)
    {

        $customer_details = Orders::where('order_id', $ord_id)->first();

        $user_Id = JwtHelper::getSesUserId();

        //Send notification for cancel customer
        $customer_token = Customer::where('customer.customer_id', $user_Id)->first();
        ...
        if ($customer_token->token != '') {
            ...
            $push = Firebase::sendSingle($key1, $message1);
        }
        if ($customer_token->mbl_token != '') {
            $message2 = [
                'title' => $title1,
                'page' => $page1,
                'portal' => $portal1,
            ...
            $key2 = $customer_token->mbl_token;
            $push = Firebase::sendSingleMbl($key2, $message2);
        }

        $get_data = GlobalHelper::notification_create($title1, $body1, 2, $user_Id, $customer_details->created_by, $module1, $page1, "website", $data1, $random_id2);
        $get_data = GlobalHelper::notification_create($title1, $body1, 2, $user_Id, $customer_details->created_by, $module1, $page1, "mobile", $data2, $random_id3);
    }
            ...
            if (!empty($id)) {
                ...
                if ($type == "rejected") {
                    ...
                    if ($request->service_id == 3) {
                        ...
                        $orderItemStatus = OrderItemStage::find($taskManDetails->orderitem_stage_id);
                        $orderItemStatus->qc_on = Server::getDateTime();
                        $orderItemStatus->qc_by = JwtHelper::getSesUserId();
                        $orderItemStatus->qc_status = 2;
                        $orderItemStatus->save();
    ...
    public function invoice_view(Request $request, $order_item_id)
    {
        try {
            ...
            $myorderView = OrderItems::where('order_items.bill_no', $get_billno->bill_no)
            ...
            $count = $myorderView->count();

            $myorderView->orderBy('order_items.order_id', 'desc');
            $myorderView = $myorderView->get();
            $count = $myorderView->count();
            if ($count > 0) {
                $final = [];
                $sum = 0;
                foreach ($myorderView as $value) {
                    $ary = [];
                    $ary['order_id'] = $value['order_id'];
                    $ary['order_code'] = $value['order_code'];
            ...
            if (!empty($final)) {
                ...
                return response()->json([
                    'keyword' => 'success',
                    'message' => __('Invoice viewed successfully'),
                    'data' => $final,
                    'no_of_items' => $count,
                    'net_amount' => $sum,
