...
class TicketWebsiteController extends Controller
{
    public function ticket_list(Request $request)
    {
        try {
            Log::channel("ticketwebsite")->info('** started the ticketwebsite list method **');
            ...
            $column_search = array('tickets.tickets_id','tickets.order_items_id','tickets.subject','tickets.priority',
            ...
            if (!empty($final)) {
                ...
                return response()->json([
                    'keyword' => 'success',
                    'message' => __('Website ticket listed successfully'),
                    'data' => $final,
                    'count' => $count,
                    'opened'=> $opened,
                    'closed'=> $closed,
    ...
    public function ticket_view($id)
    {
        try {
            ...
            $ticket_view = Tickets::where('tickets_id', $id)
                ...
                ->leftjoin('orders', 'orders.order_id', '=', 'order_items.order_id')
                ->select('tickets.*', 'orders.order_code','customer.customer_id','customer.customer_first_name','customer.customer_last_name','customer.mobile_no','customer.email','customer.profile_image')->first();
            ...
            if (!empty($ticket_view)) {
                ...
                $ary['profile_image'] = $ticket_view['profile_image'];
                ...
                ->select('ticket_inbox_id','tickets_id','messages','customer_id','acl_user_id','reply_on')->get();
                $ary['history'] = $ticket_history;

                // if ($ticket_view['status'] == 3) {
                //     $ary['status'] = "Closed";
                // }
                // if ($ticket_view['status'] == 2) {
                //     $ary['status'] = "Reply";
                // }
                // if ($ticket_view['status'] == 1) {
                //     $ary['status'] = "Opened";
                // }
            ...
            if (!empty($final)) {
                ...
                return response()->json([
                    'keyword' => 'success',
                    'message' => __('Website tickets viewed successfully'),
                    'data' => $final
                ]);
            } else {
                return response()->json([
    ...
    public function ticketinboxview($id)
    {
        try {
            ...
            if (!empty($final)) {
                ...
                return response()->json([
                    'keyword' => 'success',
                    'message' => __('Website ticket inbox viewed successfully'),
                    'data' => $final
                ]);
            } else {
                return response()->json([
