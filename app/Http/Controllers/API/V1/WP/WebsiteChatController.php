...
class WebsiteChatController extends Controller
{
    ...
    public function employee_chat_list(Request $request)
    {
        ...
        if ($count > 0) {
            ...
            foreach ($get_chatlist as $value) {
                ...
                $unread_count  = $this->getUnreadCount($value['fromUserId'],$value['toUserId']);
    ...
    public function getUnreadCount($fromId,$toId)
    {
        $get_unread = Messages::whereIn('messages.fromUserId', [$fromId,$toId])->whereIn('messages.toUserId', [$toId,$fromId])->where('isread', '!=', 2)->select('messages.isread')->get();
        return $get_unread;
    }
    ...
    public function chatConversation(Request $request)
    {
        try {
            ...
            if ($count > 0) {
                ...
                return response()->json([
                    'keyword' => 'success',
                    'message' => __('Chat conversation listed successfully'),
