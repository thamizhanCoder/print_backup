...
trait PersonalizedProductTrait
{
        ...
        try {
            ...
            if (!empty($variant_details_array)) {
                ...
                foreach ($variant_details_array as $variant) {
                    ...
                    $variantInsert->customized_price = $variant['customized_price'];
            ...
            Log::channel("personalized")->error('** error occure while inserting data in product varient table **');
    ...
    public function saverelatedProducts($relatedpro, $product_id)
    {
        try {
            ...
            Log::channel("personalized")->error($exception);
            Log::channel("personalized")->error('** error occure while inserting data in related product table **');
            return response()->json([
                'error' => 'Internal server error.',
                'message' => $exception->getMessage(),
            ], 500);
        ...
        try {
            ...
            if (!empty($variant_details_array)) {
                ...
                foreach ($variant_details_array as $variant) {
                    ...
                    $variantUpdate->customized_price = $variant['customized_price'];
                                ...
                                if (!empty($prod)) {
                                    for ($i = 0; $i < count($prod); $i++) {
                                        ...
                                        for ($j = 0; $j < $sizeOfArrayChunk; $j++) {
                                            ...
                                            $mail_data['image']  = ($produc_details_img_get->thumbnail_image != '') ? env('APP_URL') . env('PERSONALIZED_URL') . $produc_details_img_get->thumbnail_image : env('APP_URL') . "avatar.jpg";
    ...
    public function getPrimaryVariantDetails($primaryVariantDetails)
    {
        ...
        if (!empty($primaryData)) {
            ...
            foreach ($primaryData as $data) {
                ...
                $primaryDataArray['image_url'] = ($data['image'] != '') ? env('APP_URL') . env('PERSONALIZED_URL') . $data['image'] : env('APP_URL') . "avatar.jpg";
    ...
    public function getProductImage($productImageData)
    {
        ...
        if (!empty($productImageData)) {
            ...
            foreach ($sortedImages as $data) {
                ...
                $imageArray['image_url'] = ($data['image'] != '') ? env('APP_URL') . env('PERSONALIZED_URL') . $data['image'] : env('APP_URL') . "avatar.jpg";
